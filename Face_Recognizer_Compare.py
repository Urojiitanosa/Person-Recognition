...
#select the base data to which you compare
file_name=input("Type file name to compare with...")
while file_name[-4:]!=".csv":
    file_name=input("File name error! Type one more time.")
...
dif_list=[]
for i in data.keys():
    dif=np.absolute(data[i]-base_data[i])
    dif_list.append(dif)

dif=np.array(dif_list)
total_difference=np.sum(dif)
print(total_difference)

if total_difference<0.3:
    print("The same person with the base data")

elif   0.3<=total_difference<0.5:
    print("Probably the same person with the base data")

else:
    print("Different person from the base data")
